...
# runtime legs (the factory mirror of tamatebako/tebako PR #566's
# ci/windows-sign-gate.sh). The repo variable WINDOWS_SIGNING_ENABLED=true
# arms Azure Trusted Signing (Authenticode) for the leg's PE artifacts
# (the runtime exe + the ruby DLL); anything else ships unsigned BY
# DESIGN (spec 00 invariant 7: unsigned stays first-class) and this step
# is a loud notice. Armed but a secret/variable unresolved is a FAST
# named failure BEFORE any signing call — never a partial release (spec
