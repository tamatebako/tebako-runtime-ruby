...
RSpec.describe MatrixComputer do
  ...
  class FakeFetcher
    def tarball_sha256(version) = "sha-#{version}"
  ...
  class FakeDiffer
    ...
    def changed_files(_before, _after) = @files

    def file_at(ref, path) = @contents.fetch([ref, path], "")
  end
  ...
  around do |example|
    ...
    %w[GITHUB_EVENT_NAME GITHUB_EVENT_PATH MATRIX_RUBY_FILTER MATRIX_ARCH_FILTER GITHUB_OUTPUT MATRIX_JSON_PATH BUILD_GRAPH_PATH].each { |k| ENV.delete(k) }
    out_file.unlink
  end
  ...
  def run_computer(platform, event:, files: [], payload: {})
    ...
    differ.files = files
    MatrixComputer.new(["--platform", platform], fetcher: fetcher, differ: differ).run
    File.read(out_file.path)
  ensure
    payload_file&.unlink
  end

  def legs(output) = output[/^run=(.+)$/, 1]

  def rubies(output) = JSON.parse(output[/^ruby-matrix=(.+)$/, 1]).map { |r| r["version"] }
  ...
  it "emits only the changed versions on a source-release pin bump" do
    differ.contents[["oldsha", "build/lib/tebako_runtime_builder/source_fetcher.rb"]] =
      'DEFAULT_RELEASE = "v0.2.13"'
    differ.contents[["newsha", "build/lib/tebako_runtime_builder/source_fetcher.rb"]] =
      'DEFAULT_RELEASE = "v0.2.14"'
    out = run_computer("macos", event: "repository_dispatch",
                                payload: { "client_payload" => { "base_sha" => "oldsha" } })
    # FakeFetcher#sha256sums is the same for both pins → no changed versions
    expect(legs(out)).to eq("false")
  end
